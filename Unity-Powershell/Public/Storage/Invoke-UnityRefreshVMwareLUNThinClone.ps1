Function Invoke-UnityRefreshVMwareLUNThinClone {
  ...
  Param (
    ...
    [Parameter(Mandatory = $true,ValueFromPipeline=$True,ValueFromPipelinebyPropertyName=$True,HelpMessage = 'VMwareLUN ID or object.')]
    [Object]$VMwareLUN,
      ...
      Switch ($VMwareLUN.GetType().Name)
      {
        ...
        "String" {
          If ($Object = Get-UnityVMwareLUN -Session $Sess -ID $VMwareLUN -ErrorAction SilentlyContinue) {
            ...
            Throw "This VMwareLUN does not exist"
          }
        }
      }
