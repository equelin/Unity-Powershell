...
Class Unitysystem {
  ...
  [Object]$uuidBase #Base value used to generate UUIDs in the host environment(e.g. OVMS host).  
...
Class Unitylun {
  ...
  [Object]$sizePreallocated #Space reserved from the pool by the LUN, for future needs to make writes more efficient.  The pool may be able to reclaim some of this if space is running low.
  [Object]$sizeAllocatedTotal #Total pool space used in the pool for the LUN.
  [Object]$dataReductionSizeSaved #Storage element saved space by data reduction, which includes savings from compression, deduplication  and advanced deduplication.
  [Object]$dataReductionPercent #Data reduction percentage is the percentage of the data that does not consume storage - the savings due to data reduction.   For example, if 1 TB of data is stored in 250 GB, the data reduction percentage is 75%. 75% data reduction percentage is equivalent   to a 4:1 data reduction ratio.
  [Float]$dataReductionRatio #Data reduction ratio.  The data reduction ratio is the ratio between the size of the data and the amount of storage actually consumed.   For example, 1TB of data consuming 250GB would have a ration of 4:1. A 4:1 data reduction ratio is equivalent to a 75% data reduction percentage.
  [Bool]$isDataReductionEnabled #True if data reduction is enabled.
  [Bool]$isAdvancedDedupEnabled #Indicates whether to enable advanced deduplication for the LUN. It can only be enabled on the all flash high end platforms  and only when the data reduction is enabled.  If data reduction is disable, this is disabled automatically.   Deduplicated data will remain as is even after advanced deduplication is disabled.   Values are:  <li>true - Advanced deduplication is enabled on this LUN.</li>  <li>false - Advanced deduplication is disabled on this LUN.</li>  </ul>
...
Class UnityPool {
  ...
  [Object]$dataReductionSizeSaved #Amount of space saved for the pool by data reduction (includes savings from compression, deduplication and advanced deduplication).
  [Object]$dataReductionPercent #Data reduction percentage is the percentage of the data that does not consume storage - the savings due to data reduction.   For example, if 1 TB of data is stored in 250 GB, the data reduction percentage is 75%. 75% data reduction percentage is equivalent   to a 4:1 data reduction ratio.
  [Float]$dataReductionRatio #Data reduction ratio.  The data reduction ratio is the ratio between the size of the data and the amount of storage actually consumed.   For example, 1TB of data consuming 250GB would have a ration of 4:1. A 4:1 data reduction ratio is equivalent to a 75% data reduction percentage.
  [Bool]$hasDataReductionEnabledLuns #(Applies if Data Reduction is supported on the system and the corresponding license is installed.)  Indicates whether the pool has any Lun that has data reduction ever turned on; Values are:  <ul>  <li>true -  Lun(s) in this pool have had or currently have data reduction enabled.</li>  <li>false - No lun(s) in this pool have ever had data reduction enabled.</li>  </ul>
  [Bool]$hasDataReductionEnabledFs #(Applies if Data Reduction is supported on the system and the corresponding license is installed.)  Indicates whether the pool has any File System that has data reduction ever turned on; Values are.  <ul>  <li>true -  File system(s) in this pool have had or currently have data reduction enabled.</li>  <li>false - No file system(s) in this pool have ever had data reduction enabled.</li>  </ul>
  [Object]$sizePreallocated #Space reserved form the pool by storage resources, for future needs to make writes more efficient.  The pool may be able to reclaim some of this if space is running low. This will be the sum of the   sizePreallocated values of each storage resource in the pool.
...
Class UnitystorageResource {
  ...
  [ReplicationTypeEnum]$syncReplicationType #Sync Replication type.
  [Object]$sizePreallocated #Space reserved from the pool by the storage resource, for future needs to make writes more efficient.  The pool may be able to reclaim some of this if space is running low.
  [Object]$sizeAllocatedTotal #Total space used in the pool for the storage resource.
  [DedupStatusEnum]$advancedDedupStatus #Deduplication status for the storage resource.
  [DataReductionStatusEnum]$dataReductionStatus #DataReduction status for the storage resource.
  [Object]$dataReductionSizeSaved #Storage resource saved space by data reduction (includes savings from compression and deduplication).
  [Object]$dataReductionPercent #Data reduction percentage is the percentage of the data that does not consume storage - the savings due to data reduction.   For example, if 1 TB of data is stored in 250 GB, the data reduction percentage is 75%. 75% data reduction percentage is equivalent   to a 4:1 data reduction ratio.
  [Float]$dataReductionRatio #Data reduction ratio.  The data reduction ratio is the ratio between the size of the data and the amount of storage actually consumed.   For example, 1TB of data consuming 250GB would have a ration of 4:1. A 4:1 data reduction ratio is equivalent to a 75% data reduction percentage.
...
Class UnitynasServer {
  ...
  [Bool]$isBackupOnly #Indicates whether the NAS server is used as backup only. Only a replication destination can be set as backup.  Values are:  <ul>  <li>true - NAS server acts as backup only.</li>  <li>false - Normal NAS server.</li>  </ul>
  [ReplicationTypeEnum]$syncReplicationType #Sync Replication type.
  [Object]$fileSpaceUsed #File systems' space used.
  [Object]$dataReductionSizeSaved #Storage element saved space by data reduction, which includes savings from compression, deduplication  and advanced deduplication.
  [Object]$dataReductionPercent #Data reduction percentage is the percentage of the data that does not consume storage - the savings due to data reduction.   For example, if 1 TB of data is stored in 250 GB, the data reduction percentage is 75%. 75% data reduction percentage is equivalent   to a 4:1 data reduction ratio.
  [Float]$dataReductionRatio #Data reduction ratio.  The data reduction ratio is the ratio between the size of the data and the amount of storage actually consumed.   For example, 1TB of data consuming 250GB would have a ration of 4:1. A 4:1 data reduction ratio is equivalent to a 75% data reduction percentage.
...
Class UnityIpPort {
  ...
  [bool]$isLinkUp
  $storageProcessor
  [Bool]$isEnslaved #Indicates whether the ipPort is a port included in link aggregation or fsnPort.
}
...
Class Unityfilesystem {
  ...
  [Object]$compressionSizeSaved #Storage element saved space by inline compression
  [Object]$compressionPercent #Percent compression
  [Object]$sizePreallocated #Space reserved from the pool by the file system, for future needs to make writes more efficient.  The pool may be able to reclaim some of this if space is running low.
  [Object]$sizeAllocatedTotal #Total pool space used in the pool for the file system. This include the allocated   space and allocations for snaps and metadata.
  [Bool]$isDataReductionEnabled #True if data reduction is enabled, only applies when isThinEnabled is true.
  [Object]$dataReductionSizeSaved #Storage element saved space by data reduction, which includes savings from compression and deduplication.
  [Object]$dataReductionPercent #Data reduction percentage is the percentage of the data that does not consume storage - the savings due to data reduction.   For example, if 1 TB of data is stored in 250 GB, the data reduction percentage is 75%. 75% data reduction percentage is equivalent   to a 4:1 data reduction ratio.
  [Float]$dataReductionRatio #Data reduction ratio.  The data reduction ratio is the ratio between the size of the data and the amount of storage actually consumed.   For example, 1TB of data consuming 250GB would have a ration of 4:1. A 4:1 data reduction ratio is equivalent to a 75% data reduction percentage.
  [Bool]$isAdvancedDedupEnabled #True if data reduction is enabled, only applies when isThinEnabled is true.
  [FLRVersionEnum]$flrVersion #Indicates which File Level Retention (FLR) version is enabled on the file system.  Values are:  <ul>  <li>0 - FLR is OFF on the file system.</li>  <li>1 - Enterprise is enabled on the file system.</li>  <li>2 - Compliance is enabled on the file system.</li>  </ul>
  [Bool]$isFlrProtected #Indicates whether the file system is protected because it contains at least one protected file.  Values are:  <ul>  <li>true - file system contains at least one protected file.</li>  <li>false - file system doesn't contain protected file.</li>  </ul>
  [DateTime]$flrSoftwareClock #Indicates file level retention software clock value. It is file system clock used to track the retention date. It is initialized when an FLR-enabled file system is first mounted on a NAS Server.
  [String]$flrProtectedMaxRetention #Indicates maximum date and time that has been set on any locked file in an FLR-enabled file system. It is a string with a DateTime value that can extend beyond DateTime limitation of no long than 2038 year. Searching and filtering will use string ordering rather than time ordering.
  [DateTime]$flrMinRetention #The shortest period for which files on an FLR-enabled file system can be locked and protected from deletion.
  [DateTime]$flrDefaultRetention #The default retention period is used in an FLR-enabled file system when a file is locked and a retention period is not specified. The REST get value 40000 for this parameter means infinite days.
  [DateTime]$flrMaxRetention #The longest period for which files on an FLR-enabled file system can be locked and protected from deletion. The REST get value 40000 for this parameter means infinite days.
  [Bool]$isFlrAutoLockEnabled #Users can specify whether to automatically lock files in an FLR-enabled file system, as well as a policy interval for how long to wait after files are modified before the files are automatically locked. When enabled, auto-locked files are set with the default retention period value. This attribute indicates whether automatic lock is enabled on the file system.   Values are:  <ul>  <li>true - automatic lock is enabled on the file system.</li>  <li>false - automatic lock is disabled on the file system.</li>  </ul>
  [Bool]$isFlrAutoDeleteEnabled #Users can specify whether to automatically delete locked files from an FLR-enabled file system once their retention periods have expired. This value indicates whether automatic delete is enabled on the file system.  Values are:  <ul>  <li>true - automatic delete is enabled on the file system.</li>  <li>false - automatic delete is disabled on the file system.</li>  </ul>
  [DateTime]$flrAutoLockDelay #Indicates how long to wait after files are modified before the files are automatically locked. The default value is 1 hour.
...
Class UnitySmtpServer {
  ...
  [AuthTypeEnum]$authType #Authentication type.
  [SSLMethodEnum]$sslMethod #SSL Method.
  [Bool]$isBypassProxyEnabled #Whether to bypass the global proxy settings in supportProxy resource. By default, this value is false, and the SMTP server is accessed according to the proxy settings in the supportProxy resource. If this is set to true, the SMTP server will be accessed directly instead of using the supportProxy proxy settings.
}
...
Class UnityEthernetPort {
  ...
  [Object]$minMtu #Lower limit for Maximum Transmisson Unit (MTU) packet size that can be setup on the Ethernet port. The default is 1280 bytes per packet.
  [Object]$maxMtu #Upper limit for Maximum Transmisson Unit (MTU) packet size that can be setup on the Ethernet port. The default is 9216 bytes per packet.
}
...
Class UnityalertConfig {
  ...
  [DateTime]$callHomeSuppressionStartTime #Date and time when the call home suppression was started. The start time is current system time when user set the suppressionEndTime from null to a valid time. This is available only when callHomeSuppressionEndTime is set.
  [DateTime]$callHomeSuppressionEndTime #Date and time when the call home suppression will end. The total suppression window (the difference between suppressionStartTime and suppressionEndTime) should be more than 1 hour and less than 48 hours.If this value is not null then call home suppression is enabled, otherwise call home suppression is disabled.
...
Class UnityNfsServer {
  ...
  [Bool]$nfsv3Enabled #Indicates whether NFSv3 is enabled on the NAS server specified in the nasServer attribute. When enabled - NFS shares can be accessed  with NFSv3 protocol. When disabled - NFS shares cannot be accessed with NFSv3 protocol.  Values are: <ul> <li> true - NFSv3 is enabled on the specified NAS server.</li> <li> false - NFSv3 is disabled on the specified NAS server.</li> </ul>
}
...
Class UnityNFSShare {
  ...
  [Object[]]$readOnlyRootAccessHosts #Hosts with read-only root access to the NFS share and its  snapshots, as defined by the host resource type.
  [String]$noAccessHostsString #Hosts with no access to the NFS share or its snapshots,  defined as comma-separated list of hostnames, subnets, IPs,  domains or netgroups.
  [String]$readOnlyHostsString #Hosts with read-only access to the NFS share or its  snapshots, defined as comma-separated list of hostnames,  subnets, IPs, domains or netgroups.
  [String]$readWriteHostsString #Hosts with read-write access to the NFS share or its  snapshots, defined as comma-separated list of hostnames,  subnets, IPs, domains or netgroups.
  [String]$readOnlyRootHostsString #Hosts with read-only root access to the NFS share or its  snapshots, defined as comma-separated list of hostnames,  subnets, IPs, domains or netgroups.
  [String]$readWriteRootHostsString #Hosts with read-write root access to the NFS share or its  snapshots, defined as comma-separated list of hostnames,  subnets, IPs, domains or netgroups.
  [Object]$anonymousUID #Specifies the user ID of the anonymous account.
  [Object]$anonymousGID #Specifies the group ID of the anonymous account.
  [Object]$exportOption #Specifies the bitmask of various options of the NFS share.  Values are:  <ul>  <li> 0x0001 - hostString - if set, the access to the NFS  share per host is defined according the hostsString lists.  Otherwise, the access is defined according the hostID lists.  </li>  <li> 0x0002 - noSUID - if set, do not allow access to set  SUID. Otherwise, allow access.  </li>  </ul>
}
...
Class Unitysnap {
  ...
  [SyncSnapReplicationStatusEnum]$syncReplicationStatus #Indicates the sync replication status of this snap  Values are: <ul> <li> 0 - Not marked for replication</li>   <li> 1 - Replicated</li>   <li> 2 - Failed to replicate</li> </ul>
...
Class UnitySnapSchedule {
  ...
  $storageResources
  [DateTime]$modificationTime #Date and time when the snapshot schedule was created or last modified.
  [Bool]$isSyncReplicated #Indicates whether the snapshot schedule is synchronously replicated to the remote system.  <ul>  <li>true - Snapshot schedule is replicated.</li>  <li>false - Snapshot schedule is not replicated.</li>  </ul>  
}
...
Class UnityhostInitiator {
  ...
  [HostInitiatorSourceTypeEnum]$initiatorSourceType #Initiator source type.
  [HostInitiatorFailoverModeEnum]$failoverMode #Fail-over mode.
  [Bool]$isLunZEnabled #If lunZ is enabled.
  [Bool]$isArraySerialNumber #If true, array serial number, otherwise LUN serial number will be used for Unit Serial Number in Vital Product Data Page 0x80 in SCSI-3 interface.  
...
Class UnityalertConfigSNMPTarget {
  ...
  [String]$community #Community string used by SNMP v2c, not applicable for SNMP v3.  @engdocs  The community string is specified for SNMP v2c when creating it, it can be changed on the Alert Settings page.
...
Class UnityblockHostAccess {
  ...
  [Object]$hlu #Host LUN Identifier for the host storage group. This only represents the indentifiers of   the LUN(s) on the host and not the Snapshot LUN(s).  
...
Class UnityblockHostAccessParam {
  ...
  [Object]$hlu #Host LUN Identifier for the host storage. This is the user specified host LUN ID when access the LUN.  If not set, system will automatically generate with next available Host LUN ID.  
...
Class UnityfilesystemImportParameters {
  ...
  [Bool]$isDataReductionEnabled #Indicates whether to enable data reduction on the target file system. Values are:  <ul>  <li>true - Enable data reduction(default) </li>  <li>false - Disable data reduction </li>  </ul>
  [Bool]$isAdvancedDedupEnabled #Indicates whether advanced dedup should be enabled on the target file system.  Values are:  <ul>  <li>true - Enable advanced dedup on the target file system.</li>  <li>false - Do not enable advanced dedup on the target file system.</li>  </ul>
...
Class UnityfilesystemParameters {
  ...
  [FLRVersionEnum]$flrVersion #Indicates which File Level Retention (FLR) version is enabled on the file system.  Values are:  <ul>  <li>0 - FLR is OFF on the file system.</li>  <li>1 - Enterprise is enabled on the file system.</li>  <li>2 - Compliance is enabled on the file system.</li>  </ul>
  [DateTime]$flrMinRetention #The shortest period for which files on an FLR-enabled file system can be locked and protected from deletion.  The value plus the current date should be before date 2106-Feb-07.
  [DateTime]$flrDefaultRetention #The default retention period is used in an FLR-enabled file system when a file is locked and a retention period is not specified.  The value plus the current date should be before date 2106-Feb-07.
  [DateTime]$flrMaxRetention #The longest period for which files on an FLR-enabled file system can be locked and protected from deletion.  The value should be bigger than 0. The value plus the current date should be before date 2106-Feb-07. [Bool]$isFlrAutoLockEnabled #Users can specify whether to automatically lock files in an FLR-enabled file system, as well as a policy interval for how long to wait after files are modified before the files are automatically locked. When enabled, auto-locked files are set with the default retention period value. This attribute indicates whether automatic lock is enabled on the file system.  Values are:  <ul>  <li>true - automatic lock is enabled on the file system.</li>  <li>false - automatic lock is disabled on the file system.</li>  </ul>
  [Bool]$isFlrAutoDeleteEnabled #Users can specify whether to automatically delete locked files from an FLR-enabled file system once their retention periods have expired. This value indicates whether automatic delete is enabled on the file system.  Values are:  <ul>  <li>true - automatic delete is enabled on the file system.</li>  <li>false - automatic delete is disabled on the file system.</li>  </ul>
  [DateTime]$flrAutoLockDelay #Indicates how long to wait after files are modified before the files are automatically locked. The default value is 1 hour.
  [Bool]$isDataReductionEnabled #Indicates whether to enable data reduction for the file system. Values are:  <ul>  <li>true - Enable data reduction(default) </li>  <li>false - Disable data reduction </li>  </ul>
  [Bool]$isAdvancedDedupEnabled #Indicates whether to enable advanced dedup for the file system. Values are:  <ul>  <li>true - Enable advanced dedup </li>  <li>false - Disable advanced dedup(default) </li>  </ul>
  [Bool]$isFlrAutoLockEnabled #Users can specify whether to automatically lock files in an FLR-enabled file system, as well as a policy interval for how long to wait after files are modified before the files are automatically locked. When enabled, auto-locked files are set with the default retention period value. This attribute indicates whether automatic lock is enabled on the file system.  Values are:  <ul>  <li>true - automatic lock is enabled on the file system.</li>  <li>false - automatic lock is disabled on the file system.</li>  </ul>
...
Class UnityinterfacePortPair {
  ...
  [Object]$targetVlanId #Target VLAN ID of the interface port pair.  
...
Class UnityldapServer {
  ...
  [Bool]$autoDiscover #When true, the LDAP server addresses are obtained using DNS lookup on the LDAP authority (requires DNS to be configured).
  [Object[]]$serverAddresses #List of IP addresses of the LDAP server.
  [Object]$port #Port to connect to the LDAP server.
...
Class UnitylunParameters {
  ...
  [Bool]$isDataReductionEnabled #Indicates whether to enable data reduction for the LUN. Values are:  <ul>  <li>true - Enable data reduction(default) </li>  <li>false - Disable data reduction </li>  </ul>
  [Bool]$isAdvancedDedupEnabled #Indicates whether to enable advanced deduplication for the LUN. It can only be enabled on the all flash high end platforms  and only when the data reduction is enabled.  If data reduction is disable, this is disabled automatically.   Deduplicated data will remain as is even after advanced deduplication is disabled.   Values are:  <ul>  <li>true - Enable advanced deduplication. </li>  <li>false - Disable advanced deduplication(default). </li>
...
Class UnitypoolConsumerAllocation {
  ...
  [Object]$sizePreallocated #Space reserved from the pool by the consumer object, for future needs to make writes more efficient.   The pool may be able to reclaim some of this if space is running low.
...
Class UnityremoteSyslog {
  ...
  [SeverityEnum]$severity #The least severity level of information to record in the remoteSyslog accepts.  <br/>  <br/>  Default severity is DEBUG.
...
Class UnitysvcStorageProcessor {
  ...
  [SvcStorageProcessorStateEnum]$state #Indicates which state the storage processor is in.
...
Class UnitysystemCapacity {
  ...
  [Object]$sizePreallocated #Space reserved form the pools on the storage system.   These space is for future needs to make writes more efficient. pools may be able to reclaim some   of this if space is running low.
  [Float]$dataReductionRatio #Storage efficiency ratio of applying data reduction on the system, calculated according to the data reduction savings  (includes savings from compression and deduplication) for each block and file object (LUN/Filesystem/datastore) and  its snapshots, thus demonstrating the efficiency of data reduction.
  [Float]$overallEfficiencyRatio #System-level storage efficiency ratio, calculated by dividing the total logical capacity of the System by the actual  Used capacity on the System, This leverages the efficiency features of thin provisioning, snapshots and data reduction(compression  and deduplication).
  [Float]$snapsSavingsRatio #Storage efficiency ratio of snapshots on the system, calculated based on the capacity that would have been required  for fully provisioned copies, which demonstrates the efficiency of snapshots.  This is calculated as follows: (Total snapshot size of all snapshots as if they were full copies) / (Total allocated size of all snapshots without including data reduction savings, if any). This does not include data reduction savings or thin provisioning savings.
  [Object]$dataReductionSizeSaved #Amount of space saved by data reduction (includes savings from compression, deduplication and advanced deduplication) in the pools   on the storage system.
  [Object]$dataReductionPercent #Data reduction applied increase of storage space expressed as percentage of summary of used and data reduction   saved space size (includes space saved from compression and deduplication).
  [Object]$totalLogicalSize #Total logical provisioned capacity of primary storage objects visible to hosts (as defined by the “size” attribute  for each primary storage objects), plus the total logical provisioned capacity of all Snapshots (that is, the total capacity  that would be required if every snapshot were a fully provisioned copy instead).
  [Float]$thinSavingsRatio #Storage efficiency ratio of thin provisioned primary storage resources on the system, which demonstrates the efficiency of thin provisioning.  This is calculated as follows: (Total provisioned size of all primary storage resources) / (Total allocated size of all primary storage resources without including data reduction savings, if any). Because savings due to thin provisioning are virtual savings, this could exceed the total system capacity.  This does not include data reduction savings or snapshot savings.  
...
Class UnityuserQuota {
  ...
  [String]$unixName #Unix user name for this quota's uid.
  [String[]]$windowsNames #Windows user name(s) that map to this quota's uid.
  [String[]]$windowsSIDs #Windows SID(s) that map to this quota's uid.  
...
Class UnityvirtualDisk {
  ...
  [String]$manufacturer #Manufacturer of the virtual disk.
  [String]$model #Manufacturer's model number for the virtual disk.
...
<#
  Name: AdvancedDeduplicationUnsupportedReasonEnum
  Description: Reason why advanced deduplication is not supported on the pool.
#>
Enum AdvancedDeduplicationUnsupportedReasonEnum {
  None = 0 #Advanced deduplication is supported.
  Feature_Disabled = 1 #Advanced deduplication feature is disabled.
  License_Uninstalled = 2 #Advanced deduplication license is not installed.
  Pool_Type = 3 #Advanced deduplication is not supported because it is not Extreme Performance pool.
}
...
<#
  Name: AuthTypeEnum
  Description: Authentication type of smtp server.
#>
Enum AuthTypeEnum {
  None = 0 #Authorization protocol: None.
  Plain = 1 #Authorization protocol: PLAIN(Uses Base64 encoding).
  Login = 2 #Authorization protocol: LOGIN(Uses Base64 encoding).
  CRAM_MD5 = 3 #Authorization protocol: CRAM-MD5
  DIGEST_MD5 = 4 #Authorization protocol: DIGEST-MD5
}

<#
  Name: AutodownloadSoftwareTypeEnum
  Description: Image type of autodownloaded software.
#>
Enum AutodownloadSoftwareTypeEnum {
  Software = 0 #NDU software type.
  Firmware = 1 #Disk firmware software type.
  LanguagePack = 2 #Language pack software type.
  DynamicPUHC = 3 #Dynamic PUHC image type.
}
...
<#
  Name: BlockDedupRateEnum
  Description: Possible System Deduplication rates.
#>
Enum BlockDedupRateEnum {
  High = 1 #High deduplication rate.
  Medium = 2 #Medium deduplication rate.
  Low = 3 #Low deduplication rate.
  None = 4 #Deduplication rate is not initialized by the storage system.  Do not use this value in Modify requests: the storage system will respond with error.
}

<#
  Name: BlockDedupStateEnum
  Description: Block deduplication states.
#>
Enum BlockDedupStateEnum {
  Invalid = 0 #Invalid state.
  Active = 1 #Deduplication is active, session is ongoing.
  Idle = 2 #Enabled. Deduplication has never been run or it has partially been run but the schedule window is closed.
  Pending = 3 #Waiting to be run inside the schedule window.
  Paused = 4 #Paused by user.
  Completed = 5 #Completed deduplication session in the scheduled window.
  Off = 6 #Deduplication is disabled.
}
...
Enum CompressionUnsupportedReasonEnum {
  ...
  Filesystem_Compression_Not_Supported = 4 #Inline compression (ILC) is not supported for filesystems.
}
...
Enum DataCollectionProfileEnum {
  ...
  Minimum = 4 #Profile for minimum collection, useful when service storage space is very low.
}

<#
  Name: DataReductionStatusEnum
  Description: DataReduction status of the storage resource.
#>
Enum DataReductionStatusEnum {
  Disabled = 0 #DataReduction for the storage resource is disabled.
  Enabled = 1 #DataReduction for the storage resource is enabled.
  Mixed = 65535 #There is a mix of DataReduction enabled and disabled LUNs. This only applies to Consistency Groups.
}
...
<#
  Name: DCMigrationStateEnum
  Description: 
#>
Enum DCMigrationStateEnum {
  Normal = 1 #
  PrepareForMigration = 2 #
  SourceDisabledForIO = 3 #
  SourceMigrationInProgress = 4 #
  SourceMigrationCompleted = 5 #
  SourceAborted = 6 #
  Unknown = 2989 #
}
...
<#
  Name: DedupStatusEnum
  Description: Deduplication status of the storage resource.
#>
Enum DedupStatusEnum {
  Disabled = 0 #Deduplication for the storage resource is disabled.
  Enabled = 1 #Deduplication for the storage resource is enabled.
  Mixed = 65535 #There is a mix of deduplication enabled and disabled LUNs. This only applies to Consistency Groups.
}
...
<#
  ...
  Description: Possible drive technologies.
#>
Enum DiskTechnologyEnum {
  SAS = 1 #The Serial attached SCSI (SAS) drive can be used for the  Capacity storage tier.  This drive type provides high  performance and medium capacity storage for applications  that requires a balance between performance and capacity.
  NL_SAS = 2 #The Near-line serial attached SCSI (NL-SAS) drive can be used  for the Performance storage tier. This drive type provides  high capacity storage, but with lower overall performance  than regular SAS and Flash drives.
  SAS_FLASH_2 = 6 #The Medium Endurance Flash drive can be used for the  Extreme Performance storage pool and for the FAST Cache.
  SAS_FLASH_3 = 7 #The Low Endurance Flash drive can be used for the Extreme  Performance storage pool tier but not for the FAST Cache.
  SAS_FLASH_4 = 8 #The Read Intensive Flash drive can be used for the Extreme  Performance storage pool tier but not for the FAST Cache.
  SAS_FLASH_5 = 9 #Even lower endurance flash drive cannot be used for FAST Cache.
  Mixed = 50 #Applies only as an attribute value of an object containing mixed different types of drives.  Drive cannot have this type.
  Virtual = 99 #
  Unknown = 100 #
  Removed = 101 #
  ATA = 102 #
...
Enum DiskTierEnum {
  ...
  PerformanceTier = 1 #Tier that maps to SAS drives.
  CapacityTier = 2 #Tier that maps to NL-SAS drives.
...
<#
  Name: DiskTypeEnum
  Description: Possible drive types.
#>
Enum DiskTypeEnum {
  Unknown = 0 #Unsupported drive type.
  FC = 1 #
  Klondike_ATA = 2 #
  SATA = 3 #
  Northstar_ATA = 4 #
  SAS = 5 #The Serial attached SCSI (SAS) drive can be used for  Performance storage tier.
  FC_SSD = 6 #
  SATA_FSSD = 7 #
  SATA_FLASH = 8 #
  SAS_FLASH = 9 #The SAS Flash drive can be used for Extreme  Performance storage pool tier and the FAST Cache.
  NL_SAS = 10 #The Near-line serial attached SCSI (NL-SAS) drive type can be  used for the Capacity storage tier.
  SAS_FLASH_2 = 11 #The Medium Endurance Flash drive can be used for  the Extreme Performance storage pool and for the FAST  Cache.
  SAS_FLASH_3 = 12 #The Low Endurance Flash drive can be used for the Extreme  Performance storage pool tier but not for the FAST Cache.
  SAS_FLASH_4 = 13 #The Read Intensive Flash drive can be used for FAST Cache.
  SAS_FLASH_5 = 14 #Lowest endurance flash drives. May not be used for FAST Cache.
}
...
Enum EsrsConfigStatusEnum {
  ...
  Success_CallHome_Only = 3 #ESRS configured with only callhome support, which is just reserved but not used
  Success_One_Way = 4 #ESRS configured with one way connection
  Success_Two_Way = 5 #ESRS configured with two way connection
}

<#
  Name: EsrsLevelEnum
  Description: ESRS configuration level enum.
#>
Enum EsrsLevelEnum {
  Not_Set = 0 #ESRS level is not set.
  Callhome_Only = 1 #ESRS with callhome support only, which is just reserved but not used.
  One_Way = 2 #ESRS with one way connection support.
  Two_Way = 3 #ESRS with two way connection support.
}
...
<#
  Name: FLRVersionEnum
  Description: FLR version represents which File Level Retention (FLR) version is enabled on the file system. .
#>
Enum FLRVersionEnum {
  Off = 0 #FLR is OFF on the file system.
  Enterprise = 1 #FLR enterprise type is enabled on the file system. FLR enterprise type protects data content from changes made by users through CIFS, NFS, and FTP. It is intended for self-regulated archiving.
  Compliance = 2 #FLR compliance type is enabled on the file system. FLR compliance type protects data content from changes made by users through CIFS, NFS, and FTP, from changes made by administrators, and also meets the requirements of SEC rule 17a-4(f). It is intended to assist those companies that have to comply with regulations such as SEC rule 17a-4(f). The main difference between enterprise and compliance include compliance does write verify to ensure the quality and accuracy of the storage media recording process and prevents any modification or deletion of protected files by administrators.
}
...
<#
  Name: HealthCheckModeEnum
  Description: Health check mode
#>
Enum HealthCheckModeEnum {
  General = 0 #General health check
  NDU = 1 #Health check for software upgrade
  ODFU = 2 #Health check for drive firmware upgrade
}
...
<#
  Name: HostInitiatorFailoverModeEnum
  Description: This enumeration describes the possible failover mode of an initiator.
#>
Enum HostInitiatorFailoverModeEnum {
  Auto_Trespass = 0 #Any media access to the non owning SP is rejected.
  Passive_Not_Ready = 1 #A command failure during I/O is sent to the non owning SP.
  DMP_Mode = 2 #Quiet trespass on I/O to non owning SP.
  Passive_Always_Ready = 3 #Some commands, e.g. Test Unit Ready, returns PAR status.
  ALUA_Mode = 4 #Initiators are permitted to send I/O to a LUN regardless of which SP actually owns the LUN. This is the system default mode.
}
...
Enum HostInitiatorSourceTypeEnum {
  ...
  Open_Native = 3 #Open native (a.k.a. CLARiiON Open). This is the system default type.
...
Enum HostManageEnum {
  ...
  VMWARE = 1 #Automatically managed by an ESXi server.
...
Enum HostRegistrationTypeEnum {
  ...
  ESXAUTO = 2 #Initiator is registered automatically by an ESXi server.
}
...
Enum ImportCapabilityEnum {
  ...
  Non_importable_src_cs_not_primary = 16 #
}
...
<#
  Name: KmipServerStatusEnum
  Description: KMIP server status
#>
Enum KmipServerStatusEnum {
  Down = 0 #Server is down.
  Up = 1 #Server is up.
  Unknown = 2 #Server status is unknown.
}
...
Enum KmipStatusEnum {
  ...
  Unknown = 65535 #KMIP feature status is unknown. It cannot be determined.
}
...
<#
  Name: MirrorConditionEnum
  Description: Mirror condition
#>
Enum MirrorConditionEnum {
  Unknown = 0 #Condition of mirror is unknown.
  Master = 1 #Condition of mirror is master.
  Normal = 2 #Condition of mirror is normal.
  Admin_Fracture = 3 #Condition of mirror is admin fracture.
  Sys_Fracture = 4 #Condition of mirror is system fracture.
  Synch_Queue = 5 #Condition of mirror is sync queue.
  Syncing = 6 #Condition of mirror is syncing.
  Waiting_On_Admin = 7 #Condition of mirror is waiting on admin.
}

<#
  Name: MirrorStateEnum
  Description: Mirror state.
#>
Enum MirrorStateEnum {
  Unknown = 100 #Mirror state is unknown.
  Out_Of_Sync = 101 #Mirror state is out of sync.
  In_Sync = 102 #Mirror state is in sync.
  Consistent = 103 #Mirror state is consistent.
  Syncing = 104 #Mirror state is syncing.
}
...
Enum NFSShareDefaultAccessEnum {
  ...
  Root = 3 #Allow read write root access to the share for the hosts.
  RoRoot = 4 #Allow read only root access to the share for the hosts.
}
...
<#
  Name: NotRecommendedReasonEnum
  Description: Thin clone operations.
#>
Enum NotRecommendedReasonEnum {
  None = 1 #Snapshot could be used for requested Thin clone operation.
  NotReadOnly = 2 #Snapshot is currently in rw attached mode and cannot be used for Thin clone operation.
  AutoDeleteSet = 3 #Snapshot has auto delete set to true and cannot be used for Thin clone operation.
  ExpirationTimeSet = 4 #Snapshot has retention policy configured to remove snap in some period and cannot be used for Thin clone operation.
  SystemSnap = 5 #Snapshot is system and cannot be used for Thin clone operation.
}
...
#>
Enum PoolStateEnum {
  Initializing = 1 #The storage pool is being created.
  Ready = 2 #The storage pool is operating normally.
  Faulted = 3 #The storage pool is degraded.
  Offline = 4 #The storage pool is offline.
  Destroying = 5 #The storage pool is being deleted.
}
...
<#
  Name: QOSBurstDurationEnum
  Description: Burst Duration for the Storage Profile.
#>
Enum QOSBurstDurationEnum {
  one_minute = 0 #
  three_minutes = 1 #
  five_minutes = 2 #
}
...
<#
  Name: QOSLimitTypeEnum
  Description: Limit Type for the Storage Profile.
#>
Enum QOSLimitTypeEnum {
  Density_Per_GB = 0 #
  Absolute = 1 #
}

<#
  Name: QOSLimitUsageEnum
  Description: Limit Usage for the Storage Profile.
#>
Enum QOSLimitUsageEnum {
  Maximum_Rate = 0 #
  Maximum_Bandwidth = 1 #
  Both = 2 #
}
...
#>
Enum RaidTypeEnum {
  None = 0 #None. @engdocs VNX value 0 is "INVALID"
  ...
  RAID0 = 2 #RAID 0 is a striping RAID. No parity or mirroring protection used. RAID 0 is not used in the storage system. @engdocs VNX value 4.
  RAID1 = 3 #RAID 1 provides mirroring protection. RAID 1 is used only to create RAID groups for FAST Cache.
  RAID3 = 4 #RAID 3 is not used in the storage system. @engdocs VNX value 5
  ...
  Disk = 102 #@engdocs VNX value 2
  HotSpare = 106 #@engdocs VNX value 6
  RAIDHI5 = 108 #@engdocs VNX value 8
  LOGHI5 = 109 #@engdocs VNX value 9
  Unknown = 110 #@engdocs VNX value 10
  UNBOUND = 113 #@engdocs VNX value 13
}

<#
  Name: RebootPrivilegeEnum
  Description: Reboot for a time change is required only if the time shift exceeds a  threshold of 1000 seconds.  If a reboot is required, and allowed, on a single SP system or a system  with only one SP operating, then clients will be unable to access data  during the reboot.   @engdocs  There is another OSLS value in the iteration (Not_Initialized=255) that is not  exposed to users but only expose to engineering.
#>
...
Enum ReplicationOpStatusEnum {
  ...
  Syncing = 33809 #Replication session is syncing.
  Failed_Over_with_Sync_Mixed = 34792 #Planned failover state Mixed - Production access from destination site.
  Failed_Over_Mixed = 34793 #DR Failed over Mixed - Production access from destination site.
  Manual_Syncing_Mixed = 34794 #Sync in-progress Mixed for a manual sync session.
  Paused_Mixed = 34795 #Session is in paused mixed state.
  Idle_Mixed = 34796 #Session is idle mixed.
  Auto_Sync_Configured_Mixed = 34797 #Sync in-progress for auto sync session.
  Hibernated = 34798 #Source or destination is in hibernated state.
}
...
Enum ReplicationSessionStatusEnum {
  ...
  Hibernated = 16 #Session is hibernated.
}
...
Enum ResourcePoolFullPolicyEnum {
  Delete_All_Snaps = 0 #Opt to delete all snaps to free up pool space.
  Fail_Writes = 1 #Opt to fail writes.
}
...
<#
  Name: SnapStateEnum
  Description: Enumeration of possible snapshot states.  @engdocs It displays snap's state, ideally it should use snap health info,  but there is no health instance for snap at backend currently,  so we keep a backlog (US137796) for snap health instance in future.
#>
Enum SnapStateEnum {
...
<#
  Name: SSLMethodEnum
  Description: SSL protocols used in smtp server.
#>
Enum SSLMethodEnum {
  None = 0 #SSL protocol: None.
  StartTLS = 1 #SSL protocol feature: startTLS.  startTLS is a protocol feature which allows SSL and plain text connections to co-exist on the same port.  After sending this startTLS command, both client/server switch to TLS/SSL mode, and cannot switch back to plain text mode.
  SSL_TLS = 2 #SSL protocol: SSL.
}
...
Enum StorageResourceTypeEnum {
  ...
  vmwareiscsi = 4 #
  microsoft_hyperv = 5 #
  microsoft_exchange2007 = 6 #
  microsoft_exchange2010 = 7 #
  lun = 8 #
  VVolDatastoreFS = 9 #
  VVolDatastoreISCSI = 10 #
  shared_folder = 101 #@engdocs Value retained for legacy MR4 data on UC only.
  generic_storage = 102 #@engdocs Value retained for legacy MR4 data on UC only.
}
...
<#
  Name: SvcStorageProcessorStateEnum
  Description: Types of the storage processors state.
#>
Enum SvcStorageProcessorStateEnum {
  Normal = 0 #Indicates that SP is in normal mode.
  Service = 1 #Indicates that SP is in service mode.
  Hold_In_Reset = 2 #Indicates that SP is in hold in reset mode.
  Rebooting = 3 #Indicates that SP is rebooting.
}

<#
  Name: SyncSnapReplicationStatusEnum
  Description: These are the possible replication statuses for a snapshot.
#>
Enum SyncSnapReplicationStatusEnum {
  Not_Marked_For_Replication = 0 #Snapshot was created on destination site and will not be replicated on source.
  Replicated = 1 #This snapshot was replicated to the destination successfully.
  Failed_To_Replicate = 2 #Snapshot was created on source site while replication session was in fracture and will not be replicated on destination.
}

<#
  Name: ThinCloneOperationEnum
  Description: Thin clone operations.
#>
Enum ThinCloneOperationEnum {
  Create = 1 #Thin clone create.
  Refresh = 2 #Thin clone refresh.
}
...
Enum UpgradeSessionTypeEnum {
  ...
  Offline_Storageprocessor_Upgrade = 3 #
}
...
Enum UpgradeStatusEnum {
  ...
  Upgrade_Waiting_For_User = 8 #Upgrade session is waiting for user action to continue the upgrade
  Upgrade_Paused_Lock = 9 #Upgrade session is paused past the point of cancellation.
}
...
<#
  Name: VDMSupportedProtocolsEnum
  Description: Network sharing protocols supported by VDM.
#>
Enum VDMSupportedProtocolsEnum {
  NFS = 0 #Only NFS sharing protocol supported by VDM.
  CIFS = 1 #Only SMB (also known as CIFS) sharing protocol is supported by VDM.
  Multiprotocol = 2 #Both SMB (also known as CIFS) and NFS sharing protocols are supported by VDM. A file system can be shared via both NFS and CIFS simultaneously.
  CIFSAndNFS = 3 #Both CIFS and NFS sharing protocols are supported by VDM.   Any file system can only be share with one or the other.
}
...
<#
  Name: FcStatusEnum
  Description: Possible FC port statuses.
#>
Enum FcStatusEnum {
  Incorrect_Module = 33286 #FC port status is incorrect module.
  Unknown = 0 #FC port status is unknown.
  OK = 2 #FC port status is OK.
  Degraded = 3 #FC port status is degraded.
  Link_Up = 32784 #FC port status is link up.
  Link_Down = 32785 #FC port status is link down.
  Uninitialized = 33024 #FC port status is uninitialized.
  Empty = 33025 #FC port status is empty.
  Missing = 33026 #FC port status is missing.
  Faulted = 33027 #FC port status is faulted.
  Unavailable = 33028 #FC port status is unavailable.
  Disabled = 33029 #FC port status is disabled.
  SFP_Not_Present = 33280 #FC port status is SFP not present.
  Module_Not_Present = 33281 #FC port status is module not present.
  Port_Not_Present = 33282 #FC port status is port not present.
  Missing_SFP = 33283 #FC port status is missing SFP.
  Missing_Module = 33284 #FC port status is missing module.
  Incorrect_SFP_Type = 33285 #FC port status is incorrect SFP type.
  SFP_Read_Error = 33287 #FC port status is SFP read error.
  Unsupported_SFP = 33288 #FC port status is unsupported SFP.
  Module_Read_Error = 33289 #FC port status is module read error.
  Exceeded_Maximum_Limits = 33290 #FC port status is exceeded maximum limits.
  Module_Powered_Off = 33291 #FC port status is module powered off.
  Unsupported_Module = 33292 #FC port status is unsupported module.
  Database_Read_Error = 33293 #FC port status is database read error.
  Faulted_SFP = 33294 #FC port status is faulted SFP.
  Hardware_Fault = 33295 #FC port status is hardware fault.
  Disabled_User_Initiated = 33296 #FC port status is disabed user initiated.
  Disabled_Encryption_Required = 33297 #FC port status is disabled encryption required.
  Disabled_Hardware_Fault = 33298 #FC port status is disabled hardware fault.
}